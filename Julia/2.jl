function mark_perimetr!(r::Robot)#закрашивание периметра
    steps_ld = [0, 0] # (шаги_вниз, шаги_влево)
    steps_ld[1] = move_until_border!(r, Sud)
    steps_ld[2] = move_until_border!(r, West)
    for side in (Nord, Ost, Sud, West)
        putmarkers_until_border!(r, side)#ставит маркер до тех пор, пока не упрётся в перегородку
    end
    moves!(r, Ost, steps_ld[2])
    moves!(r, Nord, steps_ld[1])
end

function move_until_border!(r::Robot, side::HorizonSide)::Int #движится,пока не упрётся в перегородку
    n_steps = 0
